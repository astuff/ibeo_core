...
std::vector<Point3D> ObjectData2221::get_contour_points()
{
  ...
  for (Object2221 o : object_list)
  {
    ...
    {
      Point3D p3d;
      p3d.x = (double) p2d.x / 100.0;
      p3d.y = (double) p2d.y / 100.0;
      p3d.z = 0.0;
...
void ObjectData2271::parse(uint8_t *in)
{
  ...
  for (uint16_t i = 0; i < number_of_objects; i++)
  {
    ...
    offset += ( 118 + 4 );//(7 + 35 + 76);                                     //Add size of single object without contour points.
    offset += new_object.untracked_properties.number_of_contour_points * 8; //Add size of just-parsed object's untracked contour points.
    offset += new_object.tracked_properties.number_of_contour_points * 8;   //Add size of just-parsed object's tracked contour points.
  }
}
...
std::vector<IbeoObject> ObjectData2271::get_objects()
{
  ...
  for (Object2271 o : object_list)
  {
    ...
    if (o.tracked_properties_available)
    {
      ...
      io.object_box_orientation = o.tracked_properties.object_box_orientation / 100.0 * (M_PI / 180.0); // In 1/100 of a degree
    ...
    if (o.untracked_properties_available)
    {
      ...
      io.object_box_orientation = o.untracked_properties.object_box_orientation / 100.0 * (M_PI / 180.0); // In 1/100 of a degree
